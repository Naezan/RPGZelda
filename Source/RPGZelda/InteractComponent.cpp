...
void UInteractComponent::FindBestInteractable()
{
	...
	for (FHitResult Hit : Hits)
	{
		...
		if (HitActor)
		{
			if (HitActor->Implements<UInteractiveItemInterface>())
			{
...
bool UInteractComponent::Interact(AActor* InFocus)
{
	if (InFocus == nullptr)
	{
	...
	APawn* MyPawn = Cast<APawn>(GetOwner());
	IInteractiveItemInterface::Execute_Interact(InFocus, MyPawn);
	return true;
}

bool UInteractComponent::PrimaryInteract()
{
	return Interact(FocusedActor);
}
